...
class Multiview_MEP(nn.Module):
    def __init__(self, args):
        ...
        # encoding layers
        self.encoding = _ResNet(self.args.inplanes, self.args.resnet_depth, self.args.d_f, self.args, bottleneck=False)

        # transformer
        self.TF = Transformer(self.args)
        self.norm = nn.LayerNorm(self.args.d_f*3)
        self.pre_logits = nn.Identity()
        self.classifier = nn.Linear(self.args.d_f*3, self.args.num_classes)
    ...
    def forward(self, x):
        ...
        # print(tf_axial.shape, tf_sagittal.shape, tf_coronal.shape)
        tf_axial = self.pre_logits(tf_axial.permute(0, 2, 1)[:, 0])
        tf_sagittal = self.pre_logits(tf_sagittal.permute(0, 2, 1)[:, 0])
        tf_coronal = self.pre_logits(tf_coronal.permute(0, 2, 1)[:, 0])
        emb_matrix = torch.cat([tf_axial, tf_sagittal, tf_coronal], 1)
...
class Singleview_MEP(nn.Module):
    def __init__(self, args):
        super(Singleview_MEP, self).__init__()
        ...
        self.classifier = nn.Linear(self.args.d_f*2, self.args.num_classes)
    ...
    def forward(self, x):
        ...
        # axial
        x = x.clone()  # [B, 1, 193, 229, 193]
        mask_ind = np.random.choice(x.size(4), int(x.size(4) * self.args.mask_ratio), replace=False)
        masked_x = Variable((-1) * torch.ones(x.size(0), x.size(2), x.size(3)).unsqueeze(1).unsqueeze(-1), requires_grad=True).cuda()  # [B, 1, 193, 229]
        masked_x = masked_x.expand([masked_x.size(0), masked_x.size(1), masked_x.size(2), masked_x.size(3), len(mask_ind)])  # [4, 1, 193, 229, 19]
        # print("axial", mask_ind)
        encoding_x = self.encoding(x)  # [B, 256, 193]
        mask_encoding_x = self.encoding(masked_x)  # [B, 256, 19]

        emb_x = encoding_x.clone()  # [1, 256, 193]
        emb_x[:, :, mask_ind] = mask_encoding_x.clone()  # [MASK]

        attn_mask = torch.zeros(encoding_x.size(0), encoding_x.size(2))  # [B, 193]
        tf_x = self.TF(emb_x.permute(0, 2, 1), attn_mask.cuda(), [0]).permute(0, 2, 1)  # [B, 256, 193]

        output = self.classifier(tf_x)
        return output
...
class Transformer(nn.Module):
    def __init__(self, args):
        ...
        self.encoding_block = Encoding_Block(self.args.d_f, self.args.tf_num_heads, self.args.d_ff, self.args.tf_num_stack)
